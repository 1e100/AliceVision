...
set(AV_BUILD_DEPENDENCIES_PARALLEL 8
...
set(AV_USE_OPENCV ${AV_BUILD_OPENCV})
if(DEFINED ALICEVISION_USE_OPENCV)
    set(AV_USE_OPENCV ${ALICEVISION_USE_OPENCV})
endif()
...
message(STATUS "AV_USE_OPENCV: ${AV_USE_OPENCV}")
...
set(AV_PATCH_CMAKE_MINIMUM_COMMAND
        ${CMAKE_COMMAND}
        -DSOURCE_DIR=<SOURCE_DIR>
        -P ${CMAKE_SOURCE_DIR}/src/cmake/PatchCMakeMinimum.cmake
)

include(${CMAKE_CURRENT_LIST_DIR}/CudaAmxWorkaround.cmake)
av_get_cuda_amx_workaround_flags(AV_CUDA_AMX_WORKAROUND_FLAGS)
if(AV_USE_CUDA AND AV_CUDA_AMX_WORKAROUND_FLAGS)
    # Collapse -Xcompiler flags into a single argument (comma-separated) so CMake doesn't split them.
    set(_av_cuda_xcompiler_args "")
    foreach(_av_flag IN LISTS AV_CUDA_AMX_WORKAROUND_FLAGS)
        string(REPLACE "-Xcompiler=" "" _av_flag_stripped "${_av_flag}")
        if(_av_cuda_xcompiler_args)
            string(APPEND _av_cuda_xcompiler_args ",")
        endif()
        string(APPEND _av_cuda_xcompiler_args "${_av_flag_stripped}")
    endforeach()
    set(CCTAG_CUDA_NVCC_FLAGS "-DCUDA_NVCC_FLAGS:STRING=-Xcompiler=${_av_cuda_xcompiler_args}")
endif()
...
if(AV_BUILD_ZLIB)
    ...
    ExternalProject_Add(${ZLIB_TARGET}
        ...
        PATCH_COMMAND ${AV_PATCH_CMAKE_MINIMUM_COMMAND}
        CONFIGURE_COMMAND 
            ${CMAKE_COMMAND} 
...
if(AV_BUILD_FLANN)
    ...
    ExternalProject_Add(${FLANN_TARGET}
        ...
        PATCH_COMMAND ${AV_PATCH_CMAKE_MINIMUM_COMMAND}
        CONFIGURE_COMMAND 
            ${CMAKE_COMMAND} -E env PKG_CONFIG_PATH=${CMAKE_INSTALL_PREFIX}/lib/pkgconfig:${CMAKE_INSTALL_PREFIX}/lib64/pkgconfig
            ${CMAKE_COMMAND} 
            ${CMAKE_CORE_BUILD_FLAGS}
...
if(AV_BUILD_CCTAG)
    ...
    ExternalProject_Add(${CCTAG_TARGET}
        ...
        CONFIGURE_COMMAND 
            ...
            ${BOOST_CMAKE_FLAGS}
            ${CUDA_CMAKE_FLAGS}
            ${CCTAG_CUDA_NVCC_FLAGS}
            ${OPENCV_CMAKE_FLAGS}
            ${EIGEN_CMAKE_FLAGS}
            ${TBB_CMAKE_FLAGS}
            -DCMAKE_MODULE_PATH=${CMAKE_CURRENT_BINARY_DIR}/cctag/cmake
...
if(AV_BUILD_OPENIMAGEIO)
    ...
    set(OPENIMAGEIO_FFMPEG_CMAKE_FLAGS -DUSE_FFMPEG=ON)
    if(AV_BUILD_FFMPEG)
        list(APPEND OPENIMAGEIO_FFMPEG_CMAKE_FLAGS
            -DFFMPEG_ROOT=${CMAKE_INSTALL_PREFIX}
        )
    elseif(DEFINED FFMPEG_ROOT)
        list(APPEND OPENIMAGEIO_FFMPEG_CMAKE_FLAGS
    ...
    if(AV_BUILD_OPENCV)
        set(OPENIMAGEIO_OPENCV_CMAKE_FLAGS
            -DUSE_OPENCV=ON
            -DOpenCV_DIR=${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_LIBDIR}/cmake/opencv4
        )
    else()
        set(OPENIMAGEIO_OPENCV_CMAKE_FLAGS -DUSE_OPENCV=ON)
        if(DEFINED OpenCV_DIR)
            list(APPEND OPENIMAGEIO_OPENCV_CMAKE_FLAGS
                -DOpenCV_DIR=${OpenCV_DIR}
            )
        endif()
    endif()
    ...
    ExternalProject_Add(${OPENIMAGEIO_TARGET}
        ...
        CONFIGURE_COMMAND
            ...
            -DUSE_FREETYPE=OFF
            -DUSE_JXL=OFF
            ${OPENIMAGEIO_FFMPEG_CMAKE_FLAGS}
            ${OPENIMAGEIO_OPENCV_CMAKE_FLAGS}
            -DBUILD_DOCS=OFF
            -DBUILD_TESTING=OFF
...
if(AV_BUILD_SUITESPARSE)
    ExternalProject_add(gmp
        ...
        BUILD_COMMAND $(MAKE) -j${AV_BUILD_DEPENDENCIES_PARALLEL} MAKEINFO=/bin/true
        INSTALL_COMMAND $(MAKE) install MAKEINFO=/bin/true
    )
    ...
    set(SUITESPARSE_CMAKE_OPTIONS
        "-DBLAS_LIBRARIES=${BLAS_LIBRARIES} -DLAPACK_LIBRARIES=${LAPACK_LIBRARIES} -DBLAS_UNDERSCORE=ON -DCMAKE_C_FLAGS=-DBLAS_UNDERSCORE -DCMAKE_CXX_FLAGS=-DBLAS_UNDERSCORE -DCMAKE_INSTALL_PREFIX=<INSTALL_DIR>"
    )
    ...
    ExternalProject_Add(${SUITESPARSE_TARGET}
        ...
        INSTALL_DIR ${CMAKE_INSTALL_PREFIX}
        CONFIGURE_COMMAND ""
        BUILD_COMMAND   cd <BINARY_DIR> && ${SUITESPARSE_INTERNAL_MAKE_CMD} library CC=${CMAKE_C_COMPILER} CXX=${CMAKE_CXX_COMPILER} CMAKE_OPTIONS=${SUITESPARSE_CMAKE_OPTIONS}
        INSTALL_COMMAND cd <BINARY_DIR> && ${SUITESPARSE_INTERNAL_MAKE_CMD} install library INSTALL=<INSTALL_DIR> CC=${CMAKE_C_COMPILER} CXX=${CMAKE_CXX_COMPILER} CMAKE_OPTIONS=${SUITESPARSE_CMAKE_OPTIONS}
...
if(AV_BUILD_ALICEVISION)
    ...
    ExternalProject_Add(aliceVision
        ...
        -DALICEVISION_USE_OPENCV=${AV_USE_OPENCV}
